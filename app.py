...
# Устанавливаем секретный токен для проверки, что аутентификация прошла успешно.
AUTH_TOKEN = "your_secret_auth_token_123" 


st.set_page_config(page_title="🏗️ Склад обьекта", layout="wide")

# --- ФУНКЦИИ АВТОРИЗАЦИИ (ИСПРАВЛЕНИЕ: Использование query_params для сохранения сессии) ---

def logout():
    """Сбрасывает состояние аутентификации, очищает URL и перезапускает приложение."""
...
def check_password():
    # 1. Проверяем, есть ли уже токен в URL
    if st.query_params.get('auth_token') == AUTH_TOKEN:
        st.session_state['authenticated'] = True
        return True # Успешно аутентифицирован через URL
        
    # 2. Проверяем, вошел ли пользователь через форму (состояние сессии)
        ...
        with c1:
            ...
            if st.button("Войти", type="primary"):
                if username == "admin" and password == "admin": 
                    st.session_state['authenticated'] = True
                    # Добавляем токен в URL после успешного входа
    ...
    # Если authenticated = True, но токена нет (первый вход или старая сессия), добавляем его в URL
    if is_logged_in and st.query_params.get('auth_token') != AUTH_TOKEN:
        st.query_params['auth_token'] = AUTH_TOKEN
        st.rerun() # Перезапуск для отображения URL с токеном
...
# --- ФУНКЦИЯ ДЛЯ НЕЧЕТКОГО СОПОСТАВЛЕНИЯ (без изменений) ---
...
def submit_entry_callback_form(material_id, qty, user, current_pid, store, doc_number, note):
    ...
    try:
        ...
        # 3. Сохранение для кнопки "Отмена"
        st.session_state['last_shipment_id'] = shipment_id
        st.session_state['last_shipment_pid'] = current_pid 
        st.session_state['current_user'] = user 
        
        # 4. Сохранение последних введенных данных для удобства
        st.session_state[f"last_store_{current_pid}"] = store
        st.session_state[f"last_doc_{current_pid}"] = doc_number
        st.session_state[f"last_note_{current_pid}"] = note
    ...
    for i, tab in enumerate(tabs):
        ...
        with tab:
            ...
            if not data_df.empty:
                ...
                st.markdown(f"## 📈 Общий прогресс: **{overall_percent:.1%}**")
                st.progress(bar_value, text=f"Всего принято: **{total_shipped:.1f}** / План: **{total_planned:.1f}**")
                
                st.divider()

                # --- ВВОД ПРИХОДА (ИСПРАВЛЕНА ЛОГИКА ОБНОВЛЕНИЯ ПЛАН/ФАКТ) ---
                
                # Инициализация ключей для автоматической очистки
                if f'val_{pid}' not in st.session_state: st.session_state[f'val_{pid}'] = 0.0
                if f'who_{pid}' not in st.session_state: st.session_state[f'who_{pid}'] = WORKERS_LIST[0]
                if f'store_{pid}' not in st.session_state: st.session_state[f'store_{pid}'] = st.session_state.get(f"last_store_{pid}", "")
                if f'doc_{pid}' not in st.session_state: st.session_state[f'doc_{pid}'] = st.session_state.get(f"last_doc_{pid}", "")
                if f'note_{pid}' not in st.session_state: st.session_state[f'note_{pid}'] = st.session_state.get(f"last_note_{pid}", "")
                
                # Создание формы
                with st.form(key=f'shipment_form_{pid}'):
                    st.subheader("📥 Ввод нового прихода")
                    
                    # 1. Основные поля в колонках
                    c1, c2, c3 = st.columns([3, 1, 2])
                    
                    opts = dict(zip(data_df['name'], data_df['id']))
                    
                    with c1:
                        # Получаем выбранное имя материала
                        s_name = st.selectbox("Материал", list(opts.keys()), key=f"sel_{pid}")
                        s_id = opts[s_name]
                        
                        # ИСПРАВЛЕНИЕ: Получаем данные ТЕКУЩЕГО выбранного материала
                        curr = data_df[data_df['id']==s_id].iloc[0]
                        
                        # Отображение плана/факта, привязанное к curr
                    ...
                    with st.expander("📝 Дополнительные данные (Магазин, Док. №, Прим.)", expanded=False):
                        r2_c1, r2_c2 = st.columns(2)
                        
                        with r2_c1:
                            store_input = st.text_input("Магазин / Поставщик", key=f"store_{pid}")

                        with r2_c2:
                            doc_input = st.text_input("Номер документа", key=f"doc_{pid}")
                            
                        note_input = st.text_area("Примечание", height=50, key=f"note_{pid}")
                        
                    st.divider()
                    
                    # 3. Кнопка отправки внутри формы
                    submit_button = st.form_submit_button(
                        label="Внести (записать приход)", 
                        type="primary",
                ...
                # Улучшенная детализация с условным форматированием
                with st.expander("📊 Детализация (Остатки) — Нажмите, чтобы развернуть", expanded=False):
                    ...
                    for index, row in data_df.iterrows():
                        ...
                        if row['prog'] >= 1.0:
                            icon = "✅"
                            color = "green"
                            summary = "План выполнен!"
                        elif row['prog'] > 0:
                            icon = "⏳"
                            color = "orange"
                            summary = f"Осталось принять: **{ostalos:.2f} {row['unit']}**"
                        else:
                            icon = "⚪"
                            color = "gray"
                            summary = f"Осталось принять: **{ostalos:.2f} {row['unit']}**"
                        ...
                        if ostalos < 0:
                            icon = "⚠️"
                            color = "red"
                            summary = f"**Перерасход:** {abs(ostalos):.2f} {row['unit']}"
                ...
                if not hist_df.empty:
                    st.divider()
                    with st.expander("📜 История операций (Скачать)", expanded=False):
                        ...
                        # Отображение через HTML для цвета, с более чистыми заголовками
                        st.markdown(
                            display_df.rename(columns={'user_name': 'Кто', 'store': 'Магазин', 'doc_number': '№ Док.', 'note': 'Примечание', 'arrival_date': 'Дата'}).to_html(escape=False, index=False), 
